...
- `python -m app.cli set-password USERNAME` (in prod via an SSM shell + `docker exec -it`, see `docs/deployment.md`): the password is only ever read with `getpass` (never accept it as an argument), it must pass the same `validate_password_length` as signup, it hashes with `hash_password`, and it deletes that user's `AuthSession` rows. It refuses to run without a TTY (getpass would echo), so `send-command` can't be used.
